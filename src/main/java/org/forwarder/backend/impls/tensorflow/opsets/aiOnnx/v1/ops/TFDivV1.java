...
public class TFDivV1 extends TFAiOnnxOperator<Tensor<Number>> implements DivV1 {

	@Override
	public OperatorOutputs<Tensor<Number>> forward(Node node, Inputs inputs) {
		DivInputsV1<Tensor<Number>> castedOperatorInputs = new DivInputsV1<Tensor<Number>>(node, inputs);
		...
		return new DivOutputV1<Tensor<Number>>(this.div(a, b, axis, broadcast));
	}

	protected Tensor<Number> div(Tensor<Number> a, Tensor<Number> b, Long axis, Long broadcast) {
		...
		Operand<Number> operandDiv = tfOps.ops().math.div(operandA, operandB);
		return operandDiv.asOutput().tensor();
	}
