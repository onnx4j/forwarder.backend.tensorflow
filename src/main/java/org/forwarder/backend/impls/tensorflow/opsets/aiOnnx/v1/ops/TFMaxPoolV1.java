...
import org.onnx4j.opsets.domain.aiOnnx.v1.ops.AveragePoolV1.AveragePoolInputsV1;
import org.onnx4j.opsets.domain.aiOnnx.v1.ops.AveragePoolV1.AveragePoolOutputV1;
...
public class TFMaxPoolV1 extends TFAiOnnxOperator<Tensor<Number>> implements MaxPoolV1 {
	...
	public Tensor<Number> averagePool(Tensor<Number> data, String autoPad, List<Long> kernelShape, List<Long> pads,
			List<Long> strides) {
		...
		Operand<Number> value = tfOps.toNHWC(constantData);
		Operand<Number> opMaxPool = tfOps.ops().nn.maxPool(value, tfOps.ops().constant(Ints.toArray(newKernelShape)),
				tfOps.ops().constant(Ints.toArray(newStrides)),
				this.getTFPadding(data, autoPad, kernelShape, pads, strides), MaxPool.dataFormat("NHWC"));
		return tfOps.toNCHW(opMaxPool).asOutput().tensor();
	}
